...
<c:set value="全部价格,0-3000,3000-5000,5000-1万,1万-3万,3万~10万,10万~ " var="parrayvalue"></c:set>
    ...
    <div id="content">
        <div class="screen-column">
            <div class="section">
                ...
                <div class="select">
                	<c:if test="${empty info.price or info.price==0}">
	                    <span class="select-box">全部价格</span>
                	</c:if>
                	<c:if test="${not empty info.price and info.price > 0}">
	                    <span class="select-box">${priceArr[info.price] }</span>
                	</c:if>
                    <span class="arrow"></span>
                    <ul class="option-list" id="prc">
                        <li data-min="0" data-max="3000" data-id="1">0~3000</li>
                        <li data-min="3000" data-max="5000" data-id="2">3000~5000</li>
                        <li data-min="5000" data-max="10000" data-id="3">5000~1万</li>
                        <li data-min="10000" data-max="30000" data-id="4">1万~3万</li>
                        <li data-min="30000" data-max="100000" data-id="5">3万~10万</li>
                        <li data-min="100000" data-max="0" data-id="6">10万~
                    </ul>
                </div>
    ...
    $(function(){
    	...
    	$("._serach").click(function(){
    		...
    		//if(condi.trim() != ''){
    			...
    			var iac = $("#iac_id").val();
    			var city = $("#city").val();
    			location.href="${ctx}/info/list?infoType.id="+type+"&infoTypeOne.id="+type1+"&infoTypeTwo.id="+type2+"&title="+condi+"&price="+price+"&city="+city+"&infoAction.id="+iac;
    		//}
    	});
    	$(".paging").createPage({
    		pageCount:"${totalPage}",
    		current:"${currPage}",
    		backFn:function(p){
				location.href="${ctx}/info/list?infoType.id=${info.infoType.id}&infoTypeOne.id=${info.infoTypeOne.id}&infoTypeTwo.id=${info.infoTypeTwo.id}&title=${info.title}&price=${info.price}&city=${info.city}&infoAction.id=${info.infoAction.id}&p="+p;
    		}
    	});
